...
class CellGeneDataset(Dataset):
    """
    Custom PyTorch dataset for loading cell gene point cloud data.
    """
    def __init__(self, csv_file, class_to_idx, gene_to_idx):
        ...
        # Group by cell_id to improve indexing efficiency
    ...
    def __getitem__(self, idx):
        ...
        # Extract coordinates
        coords = cell_data[['normalized_x', 'normalized_y']].values
        ...
        # Combine features: (x, y, one_hot_gene_vector)
...
def collate_fn(batch):
    """
    Custom collate_fn to handle variable-length point cloud data.
    It will pad all point clouds in a batch to the same length.
    """
    features_list, labels_list = zip(*batch)
    
    # Find the maximum point cloud length in the batch
...
class PointNetClassifier(nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        # Feature extraction network (Shared MLP)
    ...
    def forward(self, x):
        # Input shape of x: (batch_size, num_points, num_features)
        # Conv1d requires: (batch_size, num_features, num_points)
        x = x.transpose(1, 2)
...
def plot_history(history, save_path):
    ...
    # Using a log scale to better visualize changes in magnitude
    ax3.set_yscale('log') 
    ax3.legend()
    ax3.grid(True)
...
# --- 6. Main Function ---
def main():
    parser = argparse.ArgumentParser(description="Train a point cloud model to classify cell types.")
    ...
    parser.add_argument('--output_dir', default='./model_output', help='Directory to save the model and results')
    ...
    # --- Data Preparation ---
    print("\nStep 1/4: Preparing data loaders...")
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        for epoch in range(args.epochs):
            ...
            # Store the current learning rate in history
