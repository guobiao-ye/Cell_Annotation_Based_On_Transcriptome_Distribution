# --- train_classifier_NO_SPATIAL.py (Ablation Study Version) ---
...
class CellGeneDataset(Dataset):
    """
    [Ablation Study Version] Custom PyTorch dataset that only loads gene category information, ignoring spatial coordinates.
    """
    ...
    def __getitem__(self, idx):
        ...
        # No longer extracting and using spatial coordinates
        ...
        # Features only contain one-hot encoded genes
        features = one_hot_genes.astype(np.float32)
...
# --- 6. Main Function ---
def main():
    parser = argparse.ArgumentParser(description="[Ablation Study] Train a point cloud model to classify cell types using only gene category information.")

    parser.add_argument('--train_csv', required=True)
    parser.add_argument('--test_csv', required=True)
    parser.add_argument('--color_map', required=True)
    parser.add_argument('--output_dir', default='./model_output_NO_SPATIAL', help='Directory to save the model and results')
    ...
    print("\nStep 1/4: Preparing [no spatial info] data loaders...")
    ...
    num_features = num_genes # The number of features is now the number of genes
    print(f"The model will use {num_features} input features (one-hot encoded genes only).")
    ...
    print("\nStep 3/4: Starting training (Ablation Study)...")

    with open(log_file_path, 'w') as log_file:
        log_file.write(f"--- Training started (Ablation study without spatial information): {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
