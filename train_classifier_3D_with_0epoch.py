...
# --- 1. Custom Dataset Class and collate_fn (Unchanged) ---
class CellGeneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features_local = np.hstack([coords, one_hot_genes]).astype(np.float32)
        label_str = cell_data['class'].iloc[0]
        label = self.class_to_idx[label_str]
        return torch.from_numpy(features_local), torch.tensor(label, dtype=torch.long)
...
class PointNetClassifier(nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        self.feature_extractor = nn.Sequential(
            nn.Conv1d(num_features, 64, 1), nn.BatchNorm1d(64), nn.ReLU(),
...
# --- 3. Training and Evaluation Functions (Unchanged, validate_epoch now also used for evaluation) ---
...
def validate_epoch(model, dataloader, criterion, device, desc="Validating"):
    model.eval()
    running_loss = 0.0
    correct_predictions = 0
    total_samples = 0
...
def plot_history(history, save_path):
    ...
    ax1.plot(epochs_range, history['train_acc'], 'o-', label='Train Accuracy')
    ...
    ax3.plot(epochs_range, history['lr'], 'o-', label='Learning Rate', color='orange')
...
# --- 5. Main Function ---
def main():
    parser = argparse.ArgumentParser(description="Train a [3D] point cloud model to classify cell types and compare performance before and after training.")
    # ... (Arguments unchanged) ...
    ...
    if 'z' not in train_df_for_meta.columns:
        print("Error: 'z' column not found in the input CSV file. Please check the data."); return
    ...
    # Use a larger batch size for evaluation to speed it up
    eval_loader_train = DataLoader(train_dataset, batch_size=args.batch_size * 2, shuffle=False, collate_fn=collate_fn)
    eval_loader_test = DataLoader(test_dataset, batch_size=args.batch_size * 2, shuffle=False, collate_fn=collate_fn)
    print(f"Data preparation complete. Found {num_classes} cell classes.")
    
    # --- Model Preparation ---
    print("\nStep 2/5: Building the model...")
    ...
    # *** New: Evaluate the baseline performance of the randomly initialized model ***
    print("\nStep 3/5: Evaluating pre-training (Epoch 0) performance baseline...")
    baseline_train_loss, baseline_train_acc = validate_epoch(model, eval_loader_train, criterion, device, desc="Baseline on Train Set")
    baseline_test_loss, baseline_test_acc = validate_epoch(model, eval_loader_test, criterion, device, desc="Baseline on Test Set")
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        # *** New: Write baseline results to the log ***
        baseline_log_str = (
            f"Epoch 0 (Baseline) | "
            f"Train Loss: {baseline_train_loss:.4f}, Train Acc: {baseline_train_acc:.4f} | "
            f"Test Loss: {baseline_test_loss:.4f}, Test Acc: {baseline_test_acc:.4f}"
        )
        print("\n" + baseline_log_str)
        log_file.write(baseline_log_str + '\n\n')
        ...
        for epoch in range(args.epochs):
            ...
            # Evaluate this epoch
            val_loss, val_acc = validate_epoch(model, eval_loader_test, criterion, device, desc="Validating on Test Set")
    ...
    # --- Results Visualization and Final Report ---
    print("\nStep 5/5: Generating results visualization and final report...")
    plot_history(history, output_dir / 'training_history.png')
    
    # *** New: Print the final comparison report ***
    print("\n--- Performance Comparison Report (Before and After Training) ---")
    print(f"{'Metric':<20} | {'Baseline (Epoch 0)':<25} | {'After 1 Epoch':<20} | {'Final Epoch':<20}")
    print("-" * 90)
    print(f"{'Train Accuracy':<20} | {baseline_train_acc:<25.4f} | {history['train_acc'][0]:<20.4f} | {history['train_acc'][-1]:<20.4f}")
    print(f"{'Test Accuracy':<20} | {baseline_test_acc:<25.4f} | {history['val_acc'][0]:<20.4f} | {history['val_acc'][-1]:<20.4f}")
    print(f"{'Train Loss':<20} | {baseline_train_loss:<25.4f} | {history['train_loss'][0]:<20.4f} | {history['train_loss'][-1]:<20.4f}")
    print(f"{'Test Loss':<20} | {baseline_test_loss:<25.4f} | {history['val_loss'][0]:<20.4f} | {history['val_loss'][-1]:<20.4f}")
    
    # Copy configuration files
    print("\nCopying configuration files to the output directory for future reference...")
    try:
        shutil.copy2(args.train_csv, output_dir / 'train_set_used.csv')
        shutil.copy2(args.test_csv, output_dir / 'test_set_used.csv')
        shutil.copy2(args.color_map, output_dir / 'gene_color_map.csv')
