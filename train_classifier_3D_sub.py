...
# --- 1. Custom Dataset Class (CellGeneDataset) (Modified) ---
class CellGeneDataset(Dataset):
    ...
    def __init__(self, csv_file, class_to_idx, gene_to_idx):
        # [MODIFIED] Force cell_id and subclass to be strings to avoid type issues
        self.df = pd.read_csv(csv_file, dtype={'cell_id': str, 'subclass': str})
    ...
    def __getitem__(self, idx):
        ...
        # [MODIFIED] Get label from the 'subclass' column
        subclass_str = cell_data['subclass'].iloc[0]
        label = self.class_to_idx[subclass_str]
        
        return torch.from_numpy(features), torch.tensor(label, dtype=torch.long)

# --- 2. Custom Data Loading Function (collate_fn) (Unchanged) ---
...
# --- 3. Model Definition (PointNet-like Classifier) (Unchanged) ---
...
def plot_history(history, save_path):
    ...
    ax3.set_title('Learning Rate Schedule'); ax3.set_xlabel('Epoch'); ax3.set_ylabel('Learning Rate'); ax3.set_yscale('log'); ax3.legend(); ax3.grid(True)
    plt.tight_layout(); plt.savefig(save_path)
    print(f"\nTraining history plot saved to: {save_path}")

# --- 6. Main Function (Modified for subclass) ---
def main():
    parser = argparse.ArgumentParser(description="Train a [3D] point cloud model to classify cell SUBCLASS.")
    # ... Argument parsing unchanged ...
    parser.add_argument('--train_csv', required=True)
    parser.add_argument('--test_csv', required=True)
    parser.add_argument('--color_map', required=True)
    parser.add_argument('--output_dir', default='./model_output_3d_subclass')
    ...
    # --- Data Preparation ---
    print("\nStep 1/4: Preparing [3D] data loaders (Target: subclass)...")
    ...
    # [MODIFIED] Use the union of 'subclass' from the training and test sets to build the mapping, avoiding KeyError
    dtype_spec = {'cell_id': str, 'subclass': str}
    train_df_meta = pd.read_csv(args.train_csv, dtype=dtype_spec)
    test_df_meta = pd.read_csv(args.test_csv, dtype=dtype_spec)

    if 'subclass' not in train_df_meta.columns or 'subclass' not in test_df_meta.columns:
        print("Error: 'subclass' column not found in the training or test set CSV file.")
        return
        
    all_subclasses_series = pd.concat([train_df_meta['subclass'], test_df_meta['subclass']]).dropna().unique()
    class_to_idx = {cls: i for i, cls in enumerate(sorted(all_subclasses_series))}
    ...
    print(f"Data preparation complete. Found {num_classes} cell subclasses and {num_genes} types of genes.")
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=args.lr_factor, patience=args.lr_patience, verbose=False)
    
    # --- Training Loop ---
    print("\nStep 3/4: Starting training...")
    with open(log_file_path, 'w') as log_file:
        log_file.write(f"--- Training started (3D mode, Target: subclass): {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
        log_file.write(f"Parameters: {vars(args)}\n"); log_file.write("-" * 50 + "\n")
        ...
        for epoch in range(args.epochs):
            ...
            log_str = (f"Epoch {epoch+1}/{args.epochs} | Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f} | "
                       f"Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f} | LR: {current_lr:.6f}")
            print(log_str)
            with open(log_file_path, 'a') as f: f.write(log_str + '\n')
